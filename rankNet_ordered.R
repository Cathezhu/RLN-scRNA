+rankNet_ordered <- function (object, slot.name = "netP", measure = c("weight",
...
+            prob.values[, j] <- NA
    ...
    df <- df %>% group_by(group) %>% arrange(group,desc(contribution.scaled))
    if (stacked) {
      gg <- ggplot(df, aes(x = reorder(name, contribution.scaled), y = contribution.scaled,
                           fill = group)) + geom_bar(stat = "identity",
                                                     width = bar.w, position =  "stack")
      if (measure == "weight") {
...
# Custom CellChat rankNet helper supplied by the author.
